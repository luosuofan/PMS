...
from application.statistic import forms
from application.statistic.models import Statistic
...
def StatisticList(request):
    ...
    if limit:
        ...
        limit = 655350000
    # 实例化查询对象
    query = Statistic.objects.filter(is_delete=False)
    # 按关键字查询
...
def StatisticAdd(request):
    ...
    # 表单验证
    form = forms.StatisticForm(dict_data);
    if form.is_valid():
        # 工单号
...
def StatisticDetail(Statistic_id):
    # 根据ID查询质检报表
    item = Statistic.objects.filter(is_delete=False, id=Statistic_id).first()
    # 查询结果判空
    if not item:
        return None
...
def StatisticUpdate(request):
    try:
        ...
        dict_data = json.loads(json_data)
        # 质检报表ID
        statistic_id = dict_data.get('id')
        # 质检报表ID判空
        if not statistic_id or int(statistic_id) <= 0:
    ...
    # 根据ID查询通知公告
    statistic = Statistic.objects.only('id').filter(id=statistic_id, is_delete=False).first()
    # 查询结果判断
    if not statistic:
        return R.failed("质检报表不存在")

    # 对象赋值
    statistic.work_order = work_order
    statistic.product_module = product_module
    statistic.product_name = product_name
    statistic.start_time = start_time
    statistic.end_time = end_time
    statistic.item_number = item_number
    statistic.examine_an_amount = examine_an_amount
    statistic.examine_a_bad_amount = examine_a_bad_amount
    statistic.examine_amount_total_amount = examine_amount_total_amount
    statistic.examine_bad_total_amount = examine_bad_total_amount
    statistic.target_pass_rate = target_pass_rate
    statistic.target_actual_pass_rate = target_actual_pass_rate
    statistic.signal = signal
    statistic.problem = problem
    statistic.action = action
    statistic.work_hours = round((end_time - start_time).total_seconds() / 3600 + 0.5)
    statistic.update_user = uid(request)
    statistic.update_time = datetime.now()
...
#
def StatisticDelete(Statistic_id):
    # 记录ID为空判断
    if not Statistic_id:
        return R.failed("记录ID不存在")
    # 分裂字符串
    list = Statistic_id.split(',')
    # 计数器
    count = 0
    # 遍历数据源
    if len(list) > 0:
        for id in list:
            # 根据ID查询记录
            statistic = Statistic.objects.only('id').filter(id=int(id), is_delete=False).first()
            # 查询结果判空
            if not statistic:
                return R.failed("意见反馈不存在")
            # 设置删除标识
            statistic.is_delete = True
            # 更新记录
            statistic.save()
            # 计数器+1
            count += 1
...
def StatisticListOfTotal(request):
    # 页码
    page = int(request.GET.get("page", 1))
    # 每页数
    ...
    if startTime and endTime:
        ...
        #sql = 'SELECT item_number,sum(examine_an_amount) AS total,sum(examine_a_bad_amount) AS badtotal FROM django_statistic WHERE is_delete = 0 AND start_time >= ' + str(startTime) + ' AND end_time <= ' + str(endTime)+ " GROUP BY item_number" + " limit " + str(limit)
        sql = "SELECT id,item_number, sum(examine_an_amount) AS total, sum(examine_a_bad_amount) AS badtotal FROM django_statistic WHERE is_delete = 0 AND start_time >= %s AND end_time <= %s GROUP BY item_number LIMIT %s"
        query = Statistic.objects.raw(sql,[startTime, endTime, limit])
        # 设置分页
        paginator = Paginator(query, limit)
    else:
        sql = "SELECT id,item_number, sum(examine_an_amount) AS total, sum(examine_a_bad_amount) AS badtotal FROM django_statistic WHERE is_delete = 0 GROUP BY item_number LIMIT %s"
        query = Statistic.objects.raw(sql,[ limit])
        paginator = Paginator(query, limit)
