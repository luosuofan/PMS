<template>
        ...
        <template slot="result" slot-scope="{row}">
          ...
          <template slot="toolbar">
            <el-button
              ...
              class="ele-btn-icon"
              @click="openEdit(null)"
              v-if="permission.includes('sys:testdata:add')">添加
            </el-button>
            <el-button
              size="small"
              type="danger"
          ...
          <template slot="action" slot-scope="{row}">
            <el-link
              ...
              icon="el-icon-edit"
              @click="openEdit(row)"
              v-if="permission.includes('sys:testdata:update')">修改
            </el-link>
            <el-popconfirm
              class="ele-action"
              title="确定要删除此数据吗？"
              ...
              <el-link
                ...
                :underline="false"
                icon="el-icon-delete"
                v-if="permission.includes('sys:testdata:delete')">删除
              </el-link>
            </el-popconfirm>
          </template>
          <!-- 状态列 -->
  ...
  import TestDataEdit from './testdata-edit';

  export default {
    name: 'testdata',
    components: {TestDataEdit},
    computed: {
    ...
    methods: {
      ...
      openEdit(row) {
        if (!row) {
          ...
          // 编辑
          this.loading = true;
          this.$http.get('/testdata/detail/' + row.id).then((res) => {
            this.loading = false;
            if (res.data.code === 0) {
      ...
      remove(row) {
        ...
        this.$http.delete('/testdata/delete/' + row.id).then(res => {
          loading.close();
          if (res.data.code === 0) {
      ...
      removeBatch() {
        ...
        this.$confirm('确定要删除选中的数据吗?', '提示', {
          ...
          const loading = this.$loading({lock: true});
          this.$http.delete('/testdata/delete/' + this.selection.map(d => d.id).join(",")).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message.success(res.data.msg);
              this.reload();
