      ...
      <ele-pro-table
        ...
        <template slot="action" slot-scope="{row}">
          <el-link
            ...
            icon="el-icon-edit"
            @click="openEdit(row)"
            v-if="permission.includes('sys:level:update')">修改
          </el-link>
          <el-popconfirm
            class="ele-action"
            title="确定要删除此职级吗？"
            ...
            <el-link
              ...
              :underline="false"
              icon="el-icon-delete"
              v-if="permission.includes('sys:level:delete')">删除
            </el-link>
          </el-popconfirm>
        </template>
        <!-- 状态列 -->
  ...
  methods: {
    ...
    openEdit(row) {
      if (!row) {
        ...
        // 编辑
        this.loading = true;
        this.$http.get('/level/detail/' + row.id).then((res) => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm('确定要删除选中的职级吗?', '提示', {
        ...
        const loading = this.$loading({lock: true});
        this.$http.delete('/level/delete/' + this.selection.map(d => d.id).join(",")).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message.success(res.data.msg);
            this.reload();
    ...
    editStatus(row) {
      const loading = this.$loading({lock: true});
      this.$http.put('/level/status', {id: row.id, status: row.status}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
        } else {
