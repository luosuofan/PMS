      ...
      <ele-pro-table
        ...
        <template slot="action" slot-scope="{row}">
          <el-link
            ...
            icon="el-icon-edit"
            @click="openEdit(row)"
            v-if="permission.includes('sys:user:update')">修改
          </el-link>
          <el-popconfirm
            class="ele-action"
            title="确定要删除此用户吗？"
  ...
  methods: {
    ...
    openEdit(row) {
      if (!row) {
        ...
        // 编辑
        this.loading = true;
        this.$http.get('/user/detail/' + row.id).then((res) => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm('确定要删除选中的用户吗?', '提示', {
        ...
        const loading = this.$loading({lock: true});
        this.$http.delete('/user/delete/' + this.selection.map(d => d.id).join(",")).then(res => {
          loading.close();
          if (res.data.code === 0) {
    ...
    editStatus(row) {
      const loading = this.$loading({lock: true});
      this.$http.put('/user/status',  {id: row.id, status: row.status}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message({type: 'success', message: res.data.msg});
        } else {
    ...
    resetPwd(row){
      const loading = this.$loading({lock: true});
      this.$http.put('/user/resetPwd', {id: row.id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message({type: 'success', message: res.data.msg});
        } else {
